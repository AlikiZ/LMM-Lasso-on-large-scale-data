...
# X-axis: lambda / lambda_max, Y-axis: time in secondsf
...
def _path_accuracy(X, y, steps, train_test_ratio, geomul, lower_bound=0.001):
    ...
    y_train = y[inds[:end]]
    y_test = y[inds[end:]]
    #at line 56 the solvers are defined: solver[0] is the SklearnCDSolver() which does not work; rest works
    myLasso = ScreeningLassoPath(ScreenDummy(), solver[1], path_lb=lower_bound, path_steps=steps, path_stepsize=geomul, path_scale='geometric')
    beta, nz_inds, scr_inds, path, _, _ = myLasso.fit(X_train.T, y_train, max_iter=1000, tol=1e-4, debug=False)
    #my line to print the weight
    print "the solver's coef shape: {0} times {1}".format(beta.shape[0], beta.shape[1])
    y_preds = myLasso.predict_path(beta, X_test.T)
    print "the shape of the predicted y matrix {0}".format(y_preds.shape)
