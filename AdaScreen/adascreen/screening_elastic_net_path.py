...
class ScreeningElasticNetPath(object):
    ...
    def get_plain_path(self, use_geom_hard_lower_bound=False):
        ...
        if self.path_scale=='geometric':
            print "the path of lambdas is displayed in case the distances are geometrically defined"
            ...
            for i in range(1, self.path_steps):
                ...
                if path[i] < self.path_lb:
                    ...
                    path = path[:i]
                    break
        #elaborate_path to enrich the lambda path in 2ways by adding: i) (10) extra steps close to the lmax ii) (3) steps greater than lmax
        elaborate_path = np.append(np.linspace(self.path_ub* self.path_stepsize, self.path_ub, 5), np.linspace(self.path_ub*1.05, self.path_ub *1.15, 2))
        alt_path = np.append(np.fliplr([elaborate_path])[0], path[2::])
    ...
    def calc_lambda_max(self, X, y):
        ...
        print "the calculated lmax {0}".format(lmax_x.dot(y))
        if lmax_x.dot(y)<0.0:
            print "the calculated lmax {0}".format(-lmax_x.dot(y))
            lmax_x = -lmax_x
    ...
    def fit(self, X, y, l2=0.0, max_iter=1000, tol=1e-6, debug=True):
        ...
        P = len(path)
        DIMS, EXMS = X.shape
        #print "the length of rows of X matrix {0}".format(DIMS)
        #print "the length of columns of X matrix {0}".format(EXMS)
        ...
        self.solver.release()
        return res, nz_inds, scr_inds, path, times_solver, times_screening, lmax
