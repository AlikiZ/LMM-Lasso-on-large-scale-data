...
def run(mSamples, everynthFeatures, dataset,inpycharm):
    ...
    if dataset == "ukbiobank":
        path = ['ukb_chr' + str(chromosome) + '_v2' for chromosome in range(1, 23)]
    ...
    print('... corr(Yhat,Ytrue):{0} for the lambda value {1}'.format(corr_ada[maxidx], lambda_path[maxidx]))
    ...
    #following part needed for comparison of LMM-Lasso with and without deploying screening rules as well as standard Lasso
    ...
    # plot, give a path to save the plot
    path = "/home/aliki/uni/moredata/testpred"
    plots.along_lambdapath(lambda_path, corr_ada, lambda_path[0], ms_error_ada, path=path)
    #plots.along_lambdapath_comparison(inpycharm, name, lambda_path, corr_ada, corr, corr_baseline, res['lmax'], ms_error,ms_error_ada, ms_error_baseline,nz_inds, nz_inds_lasso, nz_inds_baseline, res['number_screened_f'])

    print 'LMM-Lasso with deploying screening rules run'
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", help="inpycharm - True only if run on my local computer (for development)", default=False, type=bool)
